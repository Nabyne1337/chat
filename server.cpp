...
void broadcast(int shift, const std::string& name, const std::string& msg) {
    ...
    for (auto& s : subscribers) {
        if (s.shift == shift) {
            send(s.sock, data.c_str(), data.size(), 0);
        }
    }
}
...
void handleClient(SOCKET client) {
    ...
    if (method == "GET") {
        ...
        auto qp = parseQuery(query);
        int shift = std::stoi(qp["shift"]);
        std::string header =
            "HTTP/1.1 200 OK\r\n"
        ...
        while (true) {
            if (recv(client, buf, 1, MSG_PEEK) <= 0) break;
        ...
        }
        {
            ...
            subscribers.erase(
                std::remove_if(subscribers.begin(), subscribers.end(),
                    [&](const Subscriber& s) { return s.sock == client; }),
                subscribers.end()
            );
        }
        close(client);
        ...
        if (pos != std::string::npos) {
            std::string body = std::string(buf).substr(pos + 4);
